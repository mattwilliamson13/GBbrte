...
elev.comb <- ggplot(occ.all, aes(x = "All points", y=elev, fill=as.factor(occ))) +
  geom_boxplot() +
  geom_boxplot(data=occ.all, aes(x=as.factor(burn), y=elev, fill=as.factor(occ))) +
...
elev.occ.b = as.matrix(seq(floor(min(occ.all[occ.all$burn== 1,]$elev)), ceiling(max(occ.all[occ.all$burn== 1,]$medwtpcp)), length.out=nrow(occ.all[occ.all$burn== 1,])))
elev.occ.ub = as.matrix(seq(floor(min(occ.all[occ.all$burn== 0,]$elev)), ceiling(max(occ.all[occ.all$burn== 0,]$medwtpcp)), length.out=nrow(occ.all[occ.all$burn== 0,])))
...
p_elev.occ <- ggplot(prob_elev.occ) + geom_line(aes(y=p, x=elev, colour = factor(burn)), lwd=1.5)+
  geom_ribbon(aes(ymin=lo, ymax=hi, x=elev, fill = factor(burn)), alpha = 0.2)+
  scale_colour_viridis(discrete = "TRUE", option="E")+
  scale_fill_viridis(discrete = "TRUE", option="E") +
  labs(y="Posterior probability \nof occurrence", x = "Elevation (SD)") +
  theme(legend.position="none", text=element_text(size=14,  family="Times New Roman"), panel.background = element_rect(fill = "white", colour = "grey50"))
...
rr.b <- read.csv(paste0(datafolder,"reprate_model_burned_hillshade2.csv"), stringsAsFactors = FALSE)
rr.ub <- read.csv(paste0(datafolder,"reprate_model_unburned_hillshade.csv"), stringsAsFactors = FALSE)

elev.rr.b.all = as.matrix(seq(floor(min(rr.all[rr.all$burn== 1,]$elev)), ceiling(max(rr.all[rr.all$burn== 1,]$elev)), length.out=nrow(rr.all[rr.all$burn== 1,])))
elev.rr.ub.all = as.matrix(seq(floor(min(rr.all[rr.all$burn== 0,]$elev)), ceiling(max(rr.all[rr.all$burn== 0,]$elev)), length.out=nrow(rr.all[rr.all$burn== 0,])))
elev.rr.b <- as.matrix(seq(floor(min(rr.b$elev)), ceiling(max(rr.b$elev)), length.out=nrow(rr.b)))
elev.rr.ub <- as.matrix(seq(floor(min(rr.ub$elev)), ceiling(max(rr.ub$elev)), length.out=nrow(rr.ub)))
...
p_elev.all <- ggplot(prob_e.all) + geom_line(aes(y=p, x=elev, colour = factor(burn)), lwd=1.5)+
  geom_ribbon(aes(ymin=lo, ymax=hi, x=elev, fill = factor(burn)), alpha = 0.2)+
  scale_colour_viridis(discrete = "TRUE", option="E")+
  scale_fill_viridis(discrete = "TRUE", option="E") +
  labs(y="Posterior probability \nof prevalence", x = "Elevation (SD)") +
  theme(legend.position="none", text=element_text(size=14,  family="Times New Roman"), panel.background = element_rect(fill = "white", colour = "grey50"))

p_elev.comb <- ggplot(prob_e.comb) + geom_line(aes(y=p, x=elev, colour = factor(burn)), lwd=1.5)+
  geom_ribbon(aes(ymin=lo, ymax=hi, x=elev, fill = factor(burn)), alpha = 0.2)+
  scale_colour_viridis(discrete = "TRUE", option="E")+
  scale_fill_viridis(discrete = "TRUE", option="E") +
  labs(y="Posterior probability \nof prevalence", x = "Elevation (SD)") +
