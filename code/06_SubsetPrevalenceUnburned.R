#This code subsets the data for a binomial model of reporting rate. This analysis is aimed at understanding what drives local BRTE prevalence given that it occurs (based on the Occurrence analysis) on unburned sites.
...
db.rr.ub <- db.sub[db.sub$burn == 0,]
#select proper columns and remove incomplete cases
db.rr.ub <- db.rr.ub[,c(1:3, 5:7, 10, 12:15)] #drop year, tburn (because it doesn't make sense for unburned sites), and graze (because we are using the propgraze data)
db.rr.cc <- db.rr.ub[complete.cases(db.rr.ub),]
