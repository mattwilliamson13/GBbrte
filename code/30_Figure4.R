...
wtpcp.occ.b = as.matrix(seq(floor(min(occ.all[occ.all$burn== 1,]$medwtpcp)), ceiling(max(occ.all[occ.all$burn== 1,]$medwtpcp)), length.out=nrow(occ.all[occ.all$burn== 1,])))
wtpcp.occ.ub = as.matrix(seq(floor(min(occ.all[occ.all$burn== 0,]$medwtpcp)), ceiling(max(occ.all[occ.all$burn== 0,]$medwtpcp)), length.out=nrow(occ.all[occ.all$burn== 0,])))
...
p_wpcp.occ.h <- ggplot(prob_wp.occ.high) + geom_line(aes(y=p, x=wpcp, colour = factor(burn)), lwd=1.5)+
  geom_ribbon(aes(ymin=lo, ymax=hi, x=wpcp, fill = factor(burn)), alpha = 0.2)+
  scale_colour_viridis(discrete = "TRUE", option="E")+
  scale_fill_viridis(discrete = "TRUE", option="E") +
  labs(y="Posterior probability \nof occurrence", x = "Median winter \nprecipitation (SD)") +
  theme(legend.position="none", text=element_text(size=14,  family="Times New Roman"), panel.background = element_rect(fill = "white", colour = "grey50"))
...
p_wpcp.occ.mean <- ggplot(prob_wp.occ.mean) + geom_line(aes(y=p, x=wpcp, colour = factor(burn)), lwd=1.5)+
  geom_ribbon(aes(ymin=lo, ymax=hi, x=wpcp, fill = factor(burn)), alpha = 0.2)+
  scale_colour_viridis(discrete = "TRUE", option="E")+
  scale_fill_viridis(discrete = "TRUE", option="E") +
...
p_wpcp.occ.min <- ggplot(prob_wp.occ.min) + geom_line(aes(y=p, x=wpcp, colour = factor(burn)), lwd=1.5)+
  geom_ribbon(aes(ymin=lo, ymax=hi, x=wpcp, fill = factor(burn)), alpha = 0.2)+
  scale_colour_viridis(discrete = "TRUE", option="E")+
  scale_fill_viridis(discrete = "TRUE", option="E") +
  labs(y="Posterior probability", x = "Median winter \nprecipitation (SD)") +
...
wtpcp.rr.b = as.matrix(seq(floor(min(rr.all[rr.all$burn== 1,]$wtpr)), ceiling(max(rr.all[rr.all$burn== 1,]$wtpr)), length.out=nrow(rr.all[rr.all$burn== 1,])))
wtpcp.rr.ub = as.matrix(seq(floor(min(rr.all[rr.all$burn== 0,]$wtpr)), ceiling(max(rr.all[rr.all$burn== 0,]$wtpr)), length.out=nrow(rr.all[rr.all$burn== 0,])))
...
p_wpcp.rr.h <- ggplot(prob_wp.rr.high) + geom_line(aes(y=p, x=wpcp, colour = factor(burn)), lwd=1.5)+
  geom_ribbon(aes(ymin=lo, ymax=hi, x=wpcp, fill = factor(burn)), alpha = 0.2)+
  scale_colour_viridis(discrete = "TRUE", option="E")+
  scale_fill_viridis(discrete = "TRUE", option="E") +
  labs(y="Posterior probability \nof prevalence", x = "Winter precipitation (SD)") +
  theme(legend.position="none", text=element_text(size=14,  family="Times New Roman"), panel.background = element_rect(fill = "white", colour = "grey50"))
...
p_wpcp.rr.mean <- ggplot(prob_wp.rr.mean) + geom_line(aes(y=p, x=wpcp, colour = factor(burn)), lwd=1.5)+
  geom_ribbon(aes(ymin=lo, ymax=hi, x=wpcp, fill = factor(burn)), alpha = 0.2)+
  scale_colour_viridis(discrete = "TRUE", option="E")+
  scale_fill_viridis(discrete = "TRUE", option="E") +
  labs(y="Posterior probability", x = "Winter precipitation (SD)") +
  theme(legend.position="none", text=element_text(size=14,  family="Times New Roman"), panel.background = element_rect(fill = "white", colour = "grey50"))
...
p_wpcp.rr.min <- ggplot(prob_wp.rr.min) + geom_line(aes(y=p, x=wpcp, colour = factor(burn)), lwd=1.5)+
  geom_ribbon(aes(ymin=lo, ymax=hi, x=wpcp, fill = factor(burn)), alpha = 0.2)+
  scale_colour_viridis(discrete = "TRUE", option="E")+
  scale_fill_viridis(discrete = "TRUE", option="E") +
  labs(y="Posterior probability", x = "Winter precipitation (SD)") +
