...
slopes <- mcmc_intervals(posterior, regex_pars = c("Elevation", "Hillshade","Med. Winter precip.", "Med. Spring precip.", "Med. Winter precip./Total precip." ,"Grazed", "Time Since Burn", "Time Since Burn2", "Perennial grass frequency", "Spring precip.", "Winter precip.","Winter precip./Total precip."), prob=0.9)

slopes_area <- mcmc_areas(
  posterior,
  regex_pars =  c("Elevation", "Hillshade","Med. Winter precip.", "Med. Spring precip.", "Med. Winter precip./Total precip." ,"Grazed", "Time Since Burn", "Time Since Burn2", "Perennial grass frequency", "Spring precip.", "Winter precip.","Winter precip./Total precip."),
  prob = 0.89, # 80% intervals
  prob_outer = 1, # 99%
  point_est = "mean"
)
...
names(reprate_fit_b)[126:128] <- paste0("a_",range_names)
#rename canyon intercepts
can_names <- levels(db.stan$area)
names(reprate_fit_b)[129:133]<- paste0("a_",can_names)

#create lookup table for Can in Rg
cr <- db.stan[,c(1:2, 18:19)]
