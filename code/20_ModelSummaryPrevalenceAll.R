...
db.stan <- read.csv(here::here("processed_data/reprate_model_all_hillshade.csv"))
...
s <- summary(reprate_fit_all, pars=c("beta_p", "beta_o"), probs = c(0.10, 0.5, 0.90))
beta_sum <- as.data.frame(s$summary)
write.csv(beta_sum, file=paste0(outfolder,"/reprate_fit_all_slopes_hillshade.csv"))

#plot slope estimates
posterior <- as.array(reprate_fit_all)
color_scheme_set("red")
slopes <- mcmc_intervals(posterior, regex_pars = c("Elevation","Hillshade","Med. Winter Precip.", "Med. Winter precip./Total precip.", "Burned", "Grazed", "Perennial grass frequency", "Spring precip.", "Winter precip.", "Winter precip./Total precip."), prob=0.9)

slopes_area <- mcmc_areas(
  posterior,
  regex_pars =  c("Elevation","Hillshade","Med. Winter Precip.", "Med. Winter precip./Total precip.", "Burned", "Grazed", "Perennial grass frequency", "Spring precip.", "Winter precip.", "Winter precip./Total precip."),
...
ggsave(g, filename = paste0(outfolder,"/reprate_fit_all_slopes_hillshade.tiff"))

##INTERCEPTS
#rename range intercepts
range_names <- levels(db.stan$range)
names(reprate_fit_all)[344:347] <- paste0("a_",range_names)
#rename canyon intercepts
can_names <- levels(db.stan$area)
names(reprate_fit_all)[348:374] <- paste0("a_",can_names)

#create lookup table for Can in Rg
cr <- db.stan[,c(1:2, 16:17)]
...
s <- summary(reprate_fit_all, pars=c("alpha_rg"), probs = c(0.1, 0.5, 0.9))
alpha_rg_sum <- as.data.frame(s$summary)
write.csv(alpha_rg_sum, file=paste0(outfolder,"/reprate_fit_all_int_rg_hillshade.csv"))

#Create tabular canyon intercept results
s <- summary(reprate_fit_all, pars=c("alpha_can"), probs = c(0.1, 0.5, 0.90))
alpha_can_sum <- as.data.frame(s$summary)
write.csv(alpha_can_sum, file=paste0(outfolder,"/reprate_fit_all_int_can_hillshade.csv"))
...
ggsave(int_rg, filename = paste0(outfolder,"/reprate_fit_all_ints_hillshade.tiff"))
...
ggsave(int_rg_ar, filename = paste0(outfolder,"/reprate_fit_all_ints_area_hillshade.tiff"))
